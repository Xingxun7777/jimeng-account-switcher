...
**🎉 首个稳定版。** v1.5.0 在 v1.4.5 基础上标记为稳定发布版本，无代码变更。
...
- 导入数据字段白名单校验 + 域名 allowlist（仅 `.jianying.com`）
- `sanitizeCookie` 严格 domain 匹配（防正则绕过）
...
- Cookie 过期时间前置校验（过期项不写入）
- 多窗口 reload 按 windowId 分组，避免全量刷新

**体验**
- 完整 14+ 浏览器安装指南（Chrome/Edge/Brave/Opera/Vivaldi/Arc/360/QQ/搜狗/Yandex/Firefox 128+/Kiwi 等）
- Firefox 128+ 首次支持（`browser_specific_settings.gecko` + `chrome.permissions.request` 引导）
- 账号卡片显示 Session 过期、VIP 到期天数
- 导入/导出 JSON、重命名、删除账号等日常维护操作
- 导入模式三选一对话框（合并 / 覆盖 / 取消）+ Esc/Enter 键盘支持
- Shadow DOM 隔离的切换提示，不被页面 CSS 污染
- 无权限时禁用操作按钮，只留"授权"按钮
- 通信失败时 popup 显示明确错误态 + 重试按钮
...
- **`checkAllStatuses` catch 按异常类型区分处理**：旧版把 `withAccountCookies` 抛出的任何异常都 merge 成 `{valid:false, credits:null, vip:null}`，tab load timeout / reload 失败 / network 异常会让健康账号被错误标红并清空积分/VIP 缓存。v1.4.5 区分：
...
- **`importAccounts` 弱匹配不覆盖敏感字段**：此前导入去重只要 `importedUserId` 命中现有账号的 `userId`，就会 `Object.assign` 把导入对象的 cookies / nickname / avatar 全量覆盖到现有账号——恶意 JSON 伪造已知 userId 可替换账号登录凭证。v1.4.4 区分：
  - 强匹配（sessionid 相同）→ 全量合并（保留 id/userId）
  - 弱匹配（仅 importedUserId 相同但 sessionid 不同）→ 不覆盖 cookies，只更新 name/avatar/nickname 等非敏感显示字段

### Fixed

- **`saveCurrentAccount` 走 pending 恢复屏障**：v1.4.2 的 `PendingConflictError` 屏障只挡住 switch/check*，漏掉了 saveCurrentAccount。若上次 pending 恢复失败残留，用户再点"保存当前账号"会把脏/半恢复的 cookie 固化进 storage。v1.4.4 saveCurrentAccount 开头也检查 active pending，有就拒绝并提示用户等待自动恢复。
- **`credit`/`vip` 子请求 transport failure 保护**：v1.4.3 的 `unknown` 标志只保护 userInfo 请求。后续 credit/vip 子请求 5xx/断网时 `extract*Obj` 返回 null，`cachedCredits` / `cachedVip` 仍被抹成 null，抹掉最后一次已知好数据。v1.4.4 新增 `creditsUnknown` / `vipUnknown` 标志，merge 时看到这些标志就保留旧值。

## [1.4.3] - 2026-04-14

### Fixed

- **区分 auth failure 与 transport failure**：v1.4.1 的 `isAuthFailure` 把 `status=0` / `5xx` / 断网都当作 auth 失败，上游会把 `sessionValid` 写成 `false`——一次即梦服务短时抖动就能把所有账号标红。v1.4.3 引入 `isTransportFailure`，区分：
  - 真 auth 失败（401/403/errno 匹配）→ `sessionValid=false`
  - Transport 失败（断网/5xx/fetch 抛异常）→ `unknown=true`，不覆盖现有 `sessionValid`
- **`restoreCookies` 回滚路径也做 userId 校验**：失败后恢复原账号 cookie 时，旧版不校验 userId 就继续，若原账号期间 session 已失效（被挤下线等），会把当前 session 错误归属到原账号。v1.4.3 回滚后再跑一次 userInfo 校验，不匹配则标记 sessionValid=false。
- **`sanitizeImportedAccount` 保留 importedUserId 作为弱匹配线索**：v1.4.2 在去重前把 `userId` 清空，但 sessionid 变化的跨设备导入会产生重复。v1.4.3 保留 `importedUserId`（不进入去重主键，只作为弱匹配辅助信息）。
- **`withAccountCookies` 异常传播**：旧版 restoreCookies 失败静默返回，上游以为成功继续调用 fetch。v1.4.3 抛 `RestoreFailureError`，上游捕获后不继续执行业务逻辑。

## [1.4.2] - 2026-04-14

### Fixed

- **Manifest 权限冲突导致扩展无法加载**：v1.4.1 把 `https://*.jianying.com/*` 同时写入 `host_permissions` 和 `optional_host_permissions`。Chrome/Edge 对 MV3 校验严格，同一权限不能同时出现在两个字段，会导致扩展无法加载。v1.4.2 移除 `optional_host_permissions`。
- **`persistPendingRestore` 冲突检测**：v1.4.1 检测到未完成的 pending 时只 `console.warn` 就覆盖。如果当前 cookie 已是残缺/污染状态，会用垃圾覆盖本地唯一正常账号快照，**永久丢失登录态**。v1.4.2 改为 `throw PendingConflictError`，业务方必须显式处理（等待重启或手动恢复）。
- **`maybeRecoverOnWakeup` 节流漏洞**：v1.4.1 的 30s 节流会让"恢复失败后 30s 内的切换"跳过恢复，新业务带着残缺 cookie 进入。v1.4.2 先查 pending 是否存在，有残留就无视节流立即恢复。
- **`importAccounts` 去重失效**：v1.4.1 在 `sanitizeImportedAccount` 把 `userId` 强制清空，但去重逻辑仍查 `a.userId`，userId 路径永远失败。v1.4.2 去重改查 `a.importedUserId`。

## [1.4.1] - 2026-04-14

### Fixed (致命)

- **`makeLock` timeout 机制失效**：v1.4.0 的 `makeLock` 用 `chain = Promise.race(gated, timedOut).catch(...)`，30s 超时触发后 chain 立即 resolve，下一个任务获取锁——但原 `fn` 还在后台继续跑（JS Promise 无法取消），两个任务并发操作 cookie/storage，状态彻底损坏。v1.4.1 改为 `chain = gated.catch(...)`：锁的排队基于真实任务结束，timeout 只让调用方收到 rejection。
...
- **`withAccountCookies` 消费 `restoreCookies` 返回值**：v1.4.0 不看结果就继续 reload+fetch，把错号数据 merge 到当前账号。v1.4.1 restoreCookies 失败抛 `RestoreFailureError`，上游捕获不继续。
...
- **导入 JSON 的 `userId` 不可信**：恶意 JSON 可伪造 userId 污染去重。v1.4.1 导入时 `userId = ''`（只保留 `importedUserId` 作备注），首次 check 成功后由 mergeAccountStatus 用 API 真实 userId 回填。
- **`pending attempts` 不跨业务继承**：v1.4.0 新业务复用 `existing.attempts`，被旧失败次数过早触发 MAX。v1.4.1 新业务从 0 计。
...
- **`importAccounts` 去重对齐 userId 优先**：v1.4.0 只用 sessionid（跨设备导入同账号产生重复）。v1.4.1 改为 userId 优先 fallback sessionid。
...
- **`optional_host_permissions`**：manifest 显式声明运行时可请求的 host 权限，符合 Firefox MV3 规范（注：v1.4.2 因与 host_permissions 冲突已移除）。
...
- **Pending Restore 缺阶段标记的致命 bug**：v1.3.0 的 `_pendingRestore` 只记录 cookie 快照，如果崩在"切换成功但未清 pending"的窗口内，下次启动会反向恢复这次成功的切换。v1.4.0 引入三阶段状态机（`armed` → `business` → `committing`），只在 armed/business 状态恢复。
...
共修复 19 项（11 项 P0 阻塞级 + 8 项 P1 推荐级），显著提升安全性、正确性、用户体验。
