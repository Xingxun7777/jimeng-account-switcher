...
- 已有即梦页时的“同页切换”核心逻辑
- 无即梦页时的兜底创建逻辑
- 切换时不复用用户可见即梦页做身份校验，避免可见页误弹“已退出登录”
- 切换时保留已有积分/VIP 缓存；保存/刷新状态链路仍负责读取详细积分和会员到期
- Playwright Chromium 扩展回归流程：45 条用例

### 仍需真实账号/跨浏览器验证
- 即梦线上 Cookie 的真实写入 / 恢复效果仍受服务端风控和账号上限影响
- 真实 userInfo / credit / VIP 接口返回兼容性需要定期用 `.auth/` 私有凭证跑 smoke，凭证不得提交
- Firefox 140+ / 国产 Chromium / 移动浏览器未纳入自动化门禁；交付前如要声明正式支持，需要手工安装并跑保存/切换/刷新状态 smoke
